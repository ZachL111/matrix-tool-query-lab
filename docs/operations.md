...
`matrix-tool-query-lab` is centered on cli tools work in Python. The technical target is: Package a Python local lab for query analysis with seeded input scenarios, deterministic summary checks, and documented operating limits.
...
The score is deterministic. The current threshold is 182. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 5.
